...
namespace Example
{
    public class PlansFindPostExample
    {
        public void main()
        {
            
            var apiInstance = new PlansApi();
            var query = new Query(); // Query | Plan query

            try
            {
                // Find a set of plans for a Zip Code and County
                List&lt;Plan&gt; result = apiInstance.PlansFindPost(query);
                Debug.WriteLine(result);
            }
            catch (Exception e)
            {
                Debug.Print("Exception when calling PlansApi.PlansFindPost: " + e.Message );
            }
        }
    }
}
